...
func MaxPath(triangle [][]int) int {
	lastCollected := triangle[len(triangle)-1]

	for row := len(triangle) - 2; row >= 0; row-- {
		collection := []int{}
		for i, e := range triangle[row] {
			left := lastCollected[i]
			right := lastCollected[i+1]
			var larger int
			if right > left {
				larger = right
			} else {
			...
			collection = append(collection, larger+e)
		}
		lastCollected = collection
	}

	return lastCollected[0]
}
