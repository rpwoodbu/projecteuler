...
/*
  NB: This solution gets the right answer for the data presented, but is
  fundamentally flawed.  In an effort to skirt edge cases, I padded the grid
  with "1's", which are idempotent in multiplication, and I assumed would be
  safe to include in the grid.  However, as the grid includes "0's", it is
  conceivable that there could be a string of three numbers toward the edge
  of the grid that could by themselves (or in conjunction with a "1") yield
  the largest product, but do not in practice due to a "0" being in the line,
  nullifying their effect.  My algorithm will falsely discover these cases.
*/
...
/*
  ...
  [Ed: The "red" numbers are diagnogal "SE" from row 7, col 9, where row 1, col 1 is the "NW" corner.]
